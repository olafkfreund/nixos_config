...
```nix
# modules/claude-code/config-drift-detective.nix
{ config, lib, ... }:
{
  options.claude.drift-detective = {
  ...
  config = lib.mkIf config.claude.drift-detective.enable {
    # Drift detection systemd timer
    systemd.timers.drift-detection = {
      wantedBy = [ "timers.target" ];
      timerConfig = {
        OnCalendar = config.claude.drift-detective.schedule;
        Persistent = true;
      };
    };
  };
