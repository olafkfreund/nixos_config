...
```nix
# modules/claude-code/security-patrol.nix
{ config, lib, ... }:
{
  options.claude.security-patrol = {
  ...
  config = lib.mkIf config.claude.security-patrol.enable {
    # Automated security scanning systemd timer
    systemd.timers.security-patrol = {
      wantedBy = [ "timers.target" ];
      timerConfig = {
        OnCalendar = config.claude.security-patrol.schedule;
        Persistent = true;
      };
    };
  };
