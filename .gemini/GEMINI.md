# NixOS Infrastructure Hub - Gemini Code Guide
...
2. **ACT**: Execute exactly ONE tool call or shell command. Do not batch commands unless strictly dependent.
3. **REFLECT**: After receiving output, write a [REFLECTION]. Analyze if the output matches the plan's goal.
   Identify errors, edge cases, or missing context.
4. **REVISE**: If the reflection shows a deviation or failure, write a [REVISION] and update your PLAN
   before the next ACT.
...
> **Fast, Easy, Reliable NixOS Development with Gemini Code**
...
**Built-in Agents (Gemini Code Provides 60+ Agents)**:

Only the most relevant agents for NixOS development are listed below. Gemini Code includes 60+ built-in agents, but
...
**Note**: Gemini Code includes 40+ other built-in agents (sales-automator, minecraft-bukkit-pro, ios-developer, etc.)
...
- **Main README**: README.md (in GEMINI.md)
...
**Remember**: Gemini Code is optimized for speed, ease, and reliability. Use `/nix-help` for complete documentation and
...
3. **Self-Critique:** Act as a secondary "Reviewer" persona. Check the draft for logic errors,
...
If the task involves code or shell commands, simulate the execution in your head to ensure the logic
is finished and correct before you output a single line.
