...
```nix
# modules/gemini-cli/module-refactor.nix
{ config, lib, ... }:
{
  options.gemini.module-refactor = {
  ...
  config = lib.mkIf config.gemini.module-refactor.enable {
    # Code quality checks in build process
    system.activationScripts.code-quality-check = lib.mkIf config.gemini.module-refactor.check-on-build ''
