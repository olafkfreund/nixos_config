...
```nix
# modules/gemini-cli/performance-analyzer.nix
{ config, lib, ... }:
{
  options.gemini.performance-analyzer = {
  ...
  config = lib.mkIf config.gemini.performance-analyzer.enable {
    # Performance tracking and analysis
    systemd.timers.performance-analysis = lib.mkIf (config.gemini.performance-analyzer.schedule != null) {
      wantedBy = [ "timers.target" ];
      timerConfig = {
        OnCalendar = config.gemini.performance-analyzer.schedule;
        Persistent = true;
      };
    };
