# Update Gemini Code to Latest Version

You are a NixOS package update specialist. Update the gemini-cli package to the latest version following NixOS best practices.

## Task Overview

Update the Gemini Code package derivation with proper hash calculation, testing, and GitHub workflow integration.
...
**Search for:**
1. NPM package page: https://www.npmjs.com/package/@anthropic-ai/gemini-cli
2. GitHub releases: https://github.com/anthropics/gemini-cli/releases
...
cat home/development/gemini-cli/default.nix
```
...
Edit `home/development/gemini-cli/default.nix`:
...
# Try to build
nix-build -A packages.x86_64-linux.gemini-code 2>&1 | grep "got:" | awk '{print $2}'

# Method 2: Use nix-prefetch-url
cd /tmp
git clone https://github.com/anthropics/gemini-cli --branch vX.X.X --depth 1
cd gemini-cli
nix hash convert --to sri $(nix-prefetch-url --unpack "file://$(npm pack)")

# Method 3: Use prefetch-npm-deps (if available)
nix run nixpkgs#prefetch-npm-deps home/development/gemini-cli/package-lock.json
```

### Update the Hash

Edit `home/development/gemini-cli/default.nix` with correct hash:
...
- **Type:** chore
- **Priority:** medium
- **Title:** "Update Gemini Code to version X.X.X"
- **Description:**
  ```
  Update gemini-cli package from vOLD to vNEW
...
git checkout -b chore/${ISSUE_NUM}-update-gemini-cli-X.X.X

# Stage changes
git add home/development/gemini-cli/default.nix

# Commit with detailed message
git commit -m "chore(gemini-cli): update to version X.X.X (#${ISSUE_NUM})

Update Gemini Code package derivation:
- Version: OLD → X.X.X
...
# Push branch
git push -u origin chore/${ISSUE_NUM}-update-gemini-cli-X.X.X
```
...
Update Gemini Code to version X.X.X
...
gh issue create --title "Gemini Code X.X.X causing issues" --body "Description of issue..."
```
...
```bash
# Check build logs
nix-build -A packages.x86_64-linux.gemini-code --show-trace

# Check for missing dependencies
nix-build -A packages.x86_64-linux.gemini-code --keep-failed
...
```bash
# Test in nix-shell
nix-shell -p packages.x86_64-linux.gemini-code
claude --version
...
- @home/development/gemini-cli/default.nix - Current derivation
...
# 2. Update
vim home/development/gemini-cli/default.nix
...
# 4. GitHub workflow
/new_task
git checkout -b chore/123-update-gemini-cli-X.X.X
git add home/development/gemini-cli/default.nix
git commit -m "chore(gemini-cli): update to X.X.X (#123)"
git push -u origin chore/123-update-gemini-cli-X.X.X
gh pr create --fill
