...
in
{
  ...
  home.packages = [
    ...
    # to build one". Installed copies only kept working because the version
    # already on disk still shipped a prebuilt binary, so without this the
    # plugin is frozen at whatever commit that was.
    pkgs.bun
