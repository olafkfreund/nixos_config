...
in {
  # Enhanced SwayNC configuration (only if original not enabled)
  services.swaync = mkIf (cfg.core.enable && !(config.desktop.swaync.enable or false)) {
    enable = true;
    package = cfg.core.package;
  };
  
  # Configuration files
  xdg.configFile = mkIf (cfg.core.enable && !(config.desktop.swaync.enable or false)) {
