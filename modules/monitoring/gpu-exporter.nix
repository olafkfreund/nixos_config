...
in
{
  config = mkMerge [
      ...
      systemd.services.nvidia-gpu-exporter = {
        ...
        serviceConfig = {
          ...
          ExecStart = "${pkgs.writeShellScript "nvidia-gpu-exporter-wrapper" ''
            ...
            exec ${pkgs.prometheus-nvidia-gpu-exporter}/bin/nvidia_gpu_exporter --web.listen-address=:${toString cfg.network.gpuExporterPort}
          ''}";
          Restart = "on-failure";
          RestartSec = "5s";
    ...
    # Prometheus scrape configuration for GPU metrics - only on monitoring server
    (mkIf (cfg.enable && cfg.features.gpuMetrics && isMonitoringServer) {
      services.prometheus = {
        scrapeConfigs = [
          {
